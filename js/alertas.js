...
function agregarToast({ tipo, titulo, descripcion, autoCierre = true }) {
  if (!contenedorToast) return;

  // Alias para prevención → warning
  if (tipo === 'prevencion') tipo = 'warning';
...
const toastBootstrap = new bootstrap.Toast(nuevoToast);
toastBootstrap.show();
...
function mostrarToast(tipo, descripcion) {
  let titulo = 'Aviso';
  switch (tipo) {
    case 'exito': titulo = 'Éxito'; break;
    case 'error': titulo = 'Error'; break;
    case 'info': titulo = 'Información'; break;
    case 'prevencion': titulo = 'Prevención'; break;
    default: titulo = 'Aviso'; break;
  }
